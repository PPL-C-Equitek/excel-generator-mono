...
class ExcelExportRequestSerializer(serializers.Serializer):
    output_json = serializers.JSONField()
...
class ExcelExportResponseSerializer(serializers.Serializer):
    file_id = serializers.RegexField(r"^xlsx_[a-zA-Z0-9]+$")
    file_name = serializers.CharField()
    artifact_type = serializers.ChoiceField(choices=["xlsx"])
    size_bytes = serializers.IntegerField(min_value=0)
    created_at = serializers.DateTimeField()
