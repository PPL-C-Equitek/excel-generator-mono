...
class HistoryDownloadViewTest(BaseApiViewTest):
    ...
    def test_history_download_returns_403_for_anonymous_user(self):
        response = self.client.get(f"/history/{self.history.id}/download?format=csv")
    ...
    def test_history_download_returns_csv_file_for_owner(
        ...
        self.assertIn('attachment; filename="export_token.csv"', response["Content-Disposition"])
    ...
    def test_history_download_returns_excel_file_for_owner(
        ...
        response = self.client.get(f"/history/{self.history.id}/download?format=excel")
    ...
    def test_history_download_returns_404_for_missing_history_item(self):
        ...
        response = self.client.get("/history/00000000-0000-0000-0000-000000000000/download?format=csv")

        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.data["status"], "error")
        self.assertEqual(response.data["message"], "History item not found.")

    def test_history_download_rejects_invalid_format(self):
        self.client.force_authenticate(user=self.verified_user)

        response = self.client.get(f"/history/{self.history.id}/download?format=pdf")
