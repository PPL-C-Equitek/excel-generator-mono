...
describe('resendEmailActionFlow', () => {
  it('uses the Error message when the resend request rejects with an Error', async () => {
    ...
    await resendEmailActionFlow({
      ...
      isSubmitting: false,
      cooldown: 0,
      sendRequest: vi.fn().mockRejectedValue(new Error('Resend failed.')),
    ...
    expect(setErrorMessage).toHaveBeenCalledWith('Resend failed.');
